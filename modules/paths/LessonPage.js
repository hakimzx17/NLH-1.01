...
class LessonPage {
  ...
  _renderTheoryBlock(block) {
    ...
    switch (block.type) {
      ...
      case 'video':
        return this._renderVideoBlock(block);
      default:
        return '';
    }
  }
  ...
  _renderVideoBlock(block) {
    ...
    const videoId = block.videoId || this._extractYouTubeId(block.src);
    if (!videoId) return '';
    ...
    return `
      <section class="theory-block theory-video">
        <h4 class="theory-block__title">${this._escapeHtml(title)}</h4>
        <div class="theory-video__wrapper">
          <iframe
            src="${embedUrl}"
        ...
        ${block.caption ? `<p class="theory-video__caption">${this._escapeHtml(block.caption)}</p>` : ''}
      </section>
    `;
  }
  ...
  _extractYouTubeId(url) {
    ...
    for (const p of shortpatterns) {
      ...
      if (idx !== -1) {
        ...
        const id = url.slice(start, start + 11);
